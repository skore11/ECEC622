 ...
 *
 * The program illustrates the use of read-write locks. A read-write lock is like a 
 * mutex except that is provdes two lock functions:
 *
 *  1. The first lock function locks the read-write lock for reading. 
 *  2. The second lock function locks the read-write lock for writing.
 *  3. Multiple threads can simultaneously obtain the lock by calling the read-lock function.  
 *  4. Only one thread can obtain the lock by calling the write-lock function. 
 *  5. If any threads own the lock for reading, any threads that want to obtain the lock for writing will 
 *      block in the call to the write-lock function. 
 *  6. If any thread own the lock for writing, any threads that want to obtain the lock for reading or writing 
 *      will block in their respective locking functions.
 *
 *  Note: since writers must have exclusive access to the linked list, it is likely that it is much more difficult 
 *  for a writer to obtain the lock. Many implementations therefore give writers preference. 
 *
 * Compile as follows: gcc -o multi_threaded_linked_list_v2 multi_threaded_linked_list_v2.c -Wall -O3 -lpthread -lm
 *
 * Note: don't ask the compiler to restrict itself to C99 standard only. POSIX extensions are needed for read-write locks.
 *
...
typedef struct linked_list_s {
    ...
    pthread_rwlock_t rwlock;            /* Lock variable protecting linked list */
} linked_list_t;
...
void *worker(void *arg)
{
    ...
    for (i = 0; i < thread_data->num_trials; i++) {
        ...
        if ((op >= 0) && (op < 0.1)) {                  /* Insert element in list */
            pthread_rwlock_wrlock(&list->rwlock);       /* Acquire write lock */
            insert(list, r);
            pthread_rwlock_unlock(&list->rwlock);       /* Release write lock */
            num_inserts++;
        } 
            ...
            pthread_rwlock_rdlock(&list->rwlock);       /* Acquire read lock */
            is_present(thread_data->list, r);
            pthread_rwlock_unlock(&list->rwlock);       /* Release read lock */
            num_queries++;
        }
    }
...
linked_list_t *create_list(void)
{
    ...
    list->head = NULL;
    list->tail = NULL;
    pthread_rwlock_init(&list->rwlock, NULL);   /* Initialize the mutex */
    return list;
}
...
void destroy_list(linked_list_t *list)
{
    ...
    pthread_rwlock_destroy(&list->rwlock); /* Destroy mutex */
    return;
}
