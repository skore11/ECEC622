...
/* Return a random number uniformly distributed between [min, max] for multi-threaded version*/
float uniform_multi(float min, float max)
{
...
/* Return index of best performing particle in omp*/
int pso_get_best_fitness_omp(swarm_t *swarm)
{
    ...
    g = -1;
	#pragma omp parallel for private(particle, i) shared(g)
...
#pragma omp parallel private(i,j, particle, status) shared(g, function) num_threads(num_threads)
	status = 0;
    for (i = 0; i < swarm->num_particles; i++) {
        particle = &swarm->particle[i];
        particle->dim = dim; 
		...
		#pragma omp for
        for (j = 0; j < dim; j++)
           particle->x[j] = uniform_multi(xmin, xmax);

       /* Generate random particle velocity */ 
        particle->v = (float *)malloc(dim * sizeof(float));
		#pragma omp for
        for (j = 0; j < dim; j++)
            particle->v[j] = uniform_multi(-fabsf(xmax - xmin), fabsf(xmax - xmin));
    ...
    /* Get index of particle with best fitness */
    g = pso_get_best_fitness(swarm);
#pragma omp for //parallel for private(i, particle)
    for (i = 0; i < swarm->num_particles; i++) {
        particle = &swarm->particle[i];
        particle->g = g;
    }
